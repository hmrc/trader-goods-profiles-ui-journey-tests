...
package uk.gov.hmrc.test.ui.specpages.WithdrawAdvice
...
object WithdrawRequestSuccessPage extends BasePage {
  def title: String = "Request for advice withdrawn"
  def h1: String    = "Request for advice withdrawn"
  def url: String   = "/update-record/" + recordId + "/withdraw-request/success"
  ...
  private def verifyTitle(pageTitle: String): Unit =
    ...
    if (Driver.instance.getTitle != s"$pageTitle - $serviceName - GOV.UK")
      throw PageNotFoundException(
        s"Expected '$pageTitle' page, but found '${Driver.instance.getTitle}' page."
      )
